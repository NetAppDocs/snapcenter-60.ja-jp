...
. [Storage Connections]リストで、[Controller License]列を探します。
+
...
====
+
この例では、SnapManager Suiteライセンスがインストールされているため、SnapCenterライセンスを追加で設定する必要はありません。
...
. SnapCenter StandardまたはSnapManager Suiteのライセンスを探して記録します。
