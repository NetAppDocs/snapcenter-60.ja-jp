...
* Windowsホストの場合は、SnapCenter Plug-in for Windowsを選択する必要があります。
...
NOTE: NetAppでサポートされるプラグインは、Windows、Linux、AIXの各環境で使用できます。
...
インストールパス
 a| 
NetAppでサポートされているプラグインは、WindowsシステムとLinuxシステムのどちらにもインストールできます。

** Windows 用 SnapCenter Plug-ins パッケージのデフォルトパスは C ： \Program Files\NetApp\SnapManager です。
+
必要に応じて、パスをカスタマイズできます。

** SnapCenter Plug-ins Package for LinuxおよびSnapCenter Plug-ins Package for AIXの場合、デフォルトパスはです `/opt/NetApp/snapcenter`。
+
必要に応じて、パスをカスタマイズできます。
...
+
エラーがディスクスペースまたはRAMに関連している場合は、にあるweb.configファイルを更新してデフォルト値を変更できます `C:\Program Files\NetApp\SnapCenter WebApp`。エラーが他のパラメータに関連している場合は、問題を修正する必要があります。

+

NOTE: HAセットアップでSnapManager .Web.UI.dll.configを更新する場合は、両方のノードでファイルを更新し、SnapCenterアプリケーションプールを再起動する必要があります。

+
Windowsのデフォルトパス： `C:\Program Files\NetApp\SnapCenter WebApp\SnapManager.Web.UI.dll.config`

+
Linuxのデフォルトパス： `/opt/NetApp/snapcenter/SnapManagerWeb/SnapManager.Web.UI.dll.config`
